...
namespace KongQiang.DevTools.Utils.Helper
{
    internal class ConfigHelper
    {
        ...
        private ConfigHelper(string configPath)
        {
            ...
            {
                configPath = String.Format("{0}\\{1}", AppDomain.CurrentDomain.BaseDirectory.TrimEnd('/', '\\'), DevToolsEnvironment.ConfigTemplateName);

                if (!File.Exists(configPath))
                {
                    string[] filePaths = Directory.GetFiles(AppDomain.CurrentDomain.BaseDirectory, DevToolsEnvironment.ConfigTemplateName, SearchOption.AllDirectories);
                    if (filePaths.Length == 0)
                    {
                        throw new Exception("未找到配置模板文件");
                    }
